...
antenna_position = pd.read_csv(file_path, sep=r'\s+', names=column_names, header=0)
...
# ---------------------------------------------------------------
# Run the full reconstruction chain (PWF/SWF/ADF) on a single event
# and write the result to the output folder
# ---------------------------------------------------------------
def reconstruct_event(e, run_number, out_dir_root):
    ...
    x_coords = du_positions_ev['x'].astype(float).values #North
    y_coords = -du_positions_ev['y'].astype(float).values #West
...
def main():
    ...
    # Parse CLI arguments
    # ---------------------------------------------------------------
    parser = argparse.ArgumentParser(description="Run AOI reconstruction on a single local ROOT file")
    parser.add_argument("rootfile", help="Path to the local ROOT file")
    parser.add_argument("--events", type=str, default=None, help="Comma-separated list of event numbers to process")
    parser.add_argument("--all", action="store_true", help="Process every event in the file")
    ...
    # ---------------------------------------------------------------
    out_dir_root = "reconstructed_events_local"
    ...
    # ---------------------------------------------------------------
    el = EventList(args.rootfile, use_trawvoltage=True, trawvoltage_channels=[1,2,3])

    # ---------------------------------------------------------------
    # Select which (event_number, run_number) pairs to process
    # ---------------------------------------------------------------
    if args.all:
        targets = list(el.event_list)
    elif args.events:
        wanted = {int(x) for x in args.events.split(",")}
        targets = [pair for pair in el.event_list if pair[0] in wanted]
    else:
        targets = [el.event_list[0]]
    ...
    for event_number, run_number in targets:
        ...
        if e is None:
            print(f"Event {event_number} (run {run_number}) not found in {args.rootfile}")
            continue

        run_number = get_run_number(args.rootfile, e)
        print(f"Processing Run #{e.run_number}, Event #{e.event_number}")

        reconstruct_event(e, run_number, out_dir_root)
        el_all.append(e)

    # --------------------------------------
    # Print summary for all reconstructed events
    # --------------------------------------
    for e in el_all:
